...
namespace PresentOpener
{
	public class NPCDrops : GlobalNPC // This will allow the mod to edit vanilla NPC drops...
	{
		...
		{
			if (!Main.halloween && npc.lifeMax > 1 && npc.damage > 0 && !npc.friendly && npc.type != NPCID.Slimer && npc.type != NPCID.MeteorHead && npc.value > 0f && Main.rand.Next(240) == 0)
			{
			...
			if (!Main.xMas && npc.lifeMax > 1 && npc.damage > 0 && !npc.friendly && npc.type != NPCID.Slimer && npc.value > 0f && Main.rand.Next(39) == 0)
			{
